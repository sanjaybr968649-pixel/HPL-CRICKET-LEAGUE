...
function normalizeMatch(m){if(Array.isArray(m))return {id:m[0],home:teamName(m[1]),away:teamName(m[2]),homeScore:n(m[3]),awayScore:n(m[4]),homeWickets:Number(String(m[3]??'').split('/')[1])||0,awayWickets:Number(String(m[4]??'').split('/')[1])||0,result:m[5]||'',ground:m[6]||''};return {id:m?.id??m?.matchId,home:teamName(m?.home),away:teamName(m?.away),homeScore:n(m?.homeScore),awayScore:n(m?.awayScore),homeWickets:Number(m?.homeWickets)||0,awayWickets:Number(m?.awayWickets)||0,result:m?.result||'',ground:m?.ground||m?.venue||''}}
