...
function normalize(d){d=unwrap(d||{});const teams=(Array.isArray(d.teams)&&d.teams.length?d.teams:FALLBACK.teams).map(t=>Array.isArray(t)?{name:teamName(t[0]),logo:t[1]||'',captain:t[2]||'',ground:t[3]||''}:{...t,name:teamName(t.name)});const players=(Array.isArray(d.players)&&d.players.length?d.players:FALLBACK.players).map(p=>Array.isArray(p)?[...p,]:[p.name||'',p.short||p.name||'',teamName(p.team),p.place||'',p.role||'',p.batting||'',p.bowling||'',p.jersey||'',p.captain?1:0,Number(p.matches)||0,Number(p.runs)||0,Number(p.wickets)||0,Number(p.highest)||0,Number(p.fifties)||0,Number(p.hundreds)||0]);const matches=(Array.isArray(d.matches)&&d.matches.length?d.matches:FALLBACK.matches).map(m=>Array.isArray(m)?[...m]:[Number(m.id||m.matchId)||m.id||m.matchId,teamName(m.home),teamName(m.away),`${Number(m.homeScore)||0}/${Number(m.homeWickets)||0}`,`${Number(m.awayScore)||0}/${Number(m.awayWickets)||0}`,m.result||'',m.ground||'']);const raw=d.scorecards&&typeof d.scorecards==='object'?d.scorecards:{};const rows=v=>Array.isArray(v)?v.map(r=>Array.isArray(r)?[r[0]??'',r[1]??null,Number(r[2]||0)]:[r.player??r.name??'',r.runs??null,Number(r.wickets||0)]):[];const scorecards=Object.keys(raw).length?Object.fromEntries(Object.entries(raw).map(([k,v])=>{const src=v&&v.scorecard&&typeof v.scorecard==='object'?v.scorecard:v||{};return[k,{h:rows(src.h||src.home),a:rows(src.a||src.away)}]})):FALLBACK.scorecards;return{teams,players,matches,scorecards}}
async function startLive(){
  const cfg={apiKey:'AIzaSyDW1MP5MBKoz6oOHxeWWHF7Ed8UGlsJsQ8',authDomain:'hpl-cricket-league.firebaseapp.com',projectId:'hpl-cricket-league',storageBucket:'hpl-cricket-league.firebasestorage.app',messagingSenderId:'310274398112',appId:'1:310274398112:web:4122ce31fb352d4f05b6c5',measurementId:'G-PPMME44H28'};
  try{
    const [{initializeApp},{getFirestore,doc,onSnapshot},{getAuth,signInAnonymously}]=await Promise.all([
    ...
    const watch=()=>onSnapshot(doc(db,'hpl','data'),snap=>{
      if(snap.exists())HPL=normalize(snap.data());
      render(window.__page||'home');
      document.documentElement.dataset.firebase='connected';
    },async err=>{
      console.warn('Public Firestore read failed:',err);
      try{
        await signInAnonymously(auth);
        if(window.__hplRetryStarted)return;
        window.__hplRetryStarted=true;
        watch();
      }catch(authErr){
        console.warn('Anonymous Firebase access is not enabled or failed:',authErr);
        document.documentElement.dataset.firebase='fallback';
      }
    });
    watch();
  }catch(e){console.warn('Firebase startup failed:',e);document.documentElement.dataset.firebase='fallback'}
}
