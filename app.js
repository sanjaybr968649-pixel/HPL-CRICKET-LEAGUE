...
function calcPoints(matches,teams){const map={};teams.forEach(t=>map[t.name]={team:t.name,played:0,won:0,lost:0,points:0,runsFor:0,runsAgainst:0,diff:0});for(const m of matches){if(!m)continue;const status=String(m[8]||'Played').toLowerCase();if(status!=='played')continue;const a=teamName(m[1]),b=teamName(m[2]),ar=scoreNum(m[3]),br=scoreNum(m[4]);if(!map[a])map[a]={team:a,played:0,won:0,lost:0,points:0,runsFor:0,runsAgainst:0,diff:0};if(!map[b])map[b]={team:b,played:0,won:0,lost:0,points:0,runsFor:0,runsAgainst:0,diff:0};map[a].played++;map[b].played++;map[a].runsFor+=ar;map[a].runsAgainst+=br;map[b].runsFor+=br;map[b].runsAgainst+=ar;const result=String(m[5]||'').toLowerCase();if(result.includes(a.toLowerCase()+' won')){map[a].won++;map[a].points+=2;map[b].lost++}else if(result.includes(b.toLowerCase()+' won')){map[b].won++;map[b].points+=2;map[a].lost++}}return Object.values(map).map(x=>({...x,diff:x.runsFor-x.runsAgainst})).sort((a,b)=>b.points-a.points||b.diff-a.diff||b.runsFor-a.runsFor)}
function unwrap(v){if(v&&typeof v==='object'&&!Array.isArray(v)&&Array.isArray(v.__hplArray))return v.__hplArray.map(unwrap);if(Array.isArray(v))return v.map(unwrap);if(v&&typeof v==='object')return Object.fromEntries(Object.entries(v).map(([k,x])=>[k,unwrap(x)]));return v}
function normalize(d){d=unwrap(d||{});const rawTeams=Array.isArray(d.teams)?d.teams:[],rawPlayers=Array.isArray(d.players)?d.players:[],rawMatches=Array.isArray(d.matches)?d.matches:[];const teamNorm=t=>Array.isArray(t)?{name:teamName(t[0]),logo:t[1]||'',captain:t[2]||'',ground:t[3]||''}:{...t,name:teamName(t?.name)};const playerNorm=p=>Array.isArray(p)?[...p]:[p?.name||'',p?.short||p?.name||'',teamName(p?.team),p?.place||'',p?.role||'',p?.batting||'',p?.bowling||'',p?.jersey||'',p?.captain?1:0,Number(p?.matches)||0,Number(p?.runs)||0,Number(p?.wickets)||0,Number(p?.highest)||0,Number(p?.fifties)||0,Number(p?.hundreds)||0];const matchNorm=m=>Array.isArray(m)?[...m]:[Number(m?.id||m?.matchId)||m?.id||m?.matchId,teamName(m?.home),teamName(m?.away),`${Number(m?.homeScore)||0}/${Number(m?.homeWickets)||0}`,`${Number(m?.awayScore)||0}/${Number(m?.awayWickets)||0}`,m?.result||'',m?.ground||m?.venue||'',m?.date||m?.matchDate||'',m?.status||((m?.result||'').toLowerCase().includes('postponed')?'Postponed':'Played')];const merge=(fb,raw,key)=>{const out=fb.map(x=>x),idx=new Map(out.map((x,i)=>[key(x),i]));for(const x of raw){const k=key(x);if(idx.has(k))out[idx.get(k)]=x;else{idx.set(k,out.length);out.push(x)}}return out};const teams=merge(FALLBACK.teams,rawTeams,t=>teamNorm(t).name.toLowerCase()).map(teamNorm);const players=merge(FALLBACK.players,rawPlayers,p=>String(playerNorm(p)[0]||playerNorm(p)[1]||'').toLowerCase()).map(playerNorm);const matches=merge(FALLBACK.matches,rawMatches,m=>String(matchNorm(m)[0]??'')).map(matchNorm);const rawCards=d.scorecards&&typeof d.scorecards==='object'?d.scorecards:{};const rows=v=>Array.isArray(v)?v.map(r=>Array.isArray(r)?[r[0]??'',r[1]??null,Number(r[2]||0)]:[r?.player??r?.name??'',r?.runs??null,Number(r?.wickets||0)]):[];const cards={...FALLBACK.scorecards,...rawCards};const scorecards=Object.fromEntries(Object.entries(cards).map(([k,v])=>{const src=v&&v.scorecard&&typeof v.scorecard==='object'?v.scorecard:v||{};return[k,{h:rows(src.h||src.home),a:rows(src.a||src.away)}]}));return{teams,players,matches,scorecards}}
const esc=x=>String(x??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]));
function logo(n){return (HPL.teams.find(t=>t.name===n)||{}).logo||({ 'Royal Kings':'royal-kings.png','Titans':'titans.png','Chasers':'chasers.png','Stars':'stars.png'}[n]||'hpl.png')}
function header(p){return `<header class="header"><div class="head"><img class="hpl-logo" src="hpl.png"><div><div class="title">HAND CRICKET PREMIER LEAGUE</div><div class="sub">Season 11 • Live Firebase Data</div></div></div></header><nav class="nav"><div class="navin">${[['home','Home'],['teams','Teams'],['players','Players'],['matches','Matches'],['points','Points Table'],['stats','Player Stats']].map(x=>`<button class="${p===x[0]?'active':''}" onclick="showPage('${x[0]}')">${x[1]}</button>`).join('')}</div></nav>`}
function mc(m){const status=m[8]||'Played';const isPlayed=String(status).toLowerCase()==='played';return `<article class="match"><small>MATCH ${m[0]}</small><div class="teamscore"><div class="t"><img src="${logo(m[1])}"><div><b>${esc(m[1])}</b><br><span class="score">${isPlayed?esc(m[3]):'—'}</span></div></div><b>VS</b><div class="t right"><div><b>${esc(m[2])}</b><br><span class="score">${isPlayed?esc(m[4]):'—'}</span></div><img src="${logo(m[2])}"></div></div><div class="result">${esc(m[5]||status)}${status!=='Played'?`<br><small>Status: ${esc(status)}</small>`:''}</div>${m[7]?`<div class="ground">📅 ${esc(m[7])}</div>`:''}<div class="ground">📍 ${esc(m[6])}</div>${isPlayed?`<button class="btn" onclick="scorecard(${m[0]})">View Scorecard</button>`:''}</article>`}
...
function scorecard(id){const m=HPL.matches.find(x=>String(x[0])===String(id)),s=HPL.scorecards[id];if(!m||!s||String(m[8]||'Played').toLowerCase()!=='played')return;document.getElementById('app').innerHTML=header('matches')+`<main class="wrap"><button class="btn" onclick="showPage('matches')">← Back</button><h2>Match ${id} Scorecard</h2>${mc(m)}${st(m[1],m[3],s.h)}${st(m[2],m[4],s.a)}</main><footer class="foot">HPL Season 11</footer>`;scrollTo(0,0)}
...
function startLive(){
  onSnapshot(hplRef, snap => {
    try {
      if (!snap.exists()) {
        HPL = FALLBACK;
      } else {
        HPL = normalize(snap.data());
      }
      if (!Array.isArray(HPL.teams)) HPL.teams = FALLBACK.teams;
      if (!Array.isArray(HPL.players)) HPL.players = FALLBACK.players;
      if (!Array.isArray(HPL.matches)) HPL.matches = FALLBACK.matches;
      if (!HPL.scorecards || typeof HPL.scorecards !== 'object') HPL.scorecards = FALLBACK.scorecards;
      render(window.__page || 'home');
    } catch (e) {
      console.error('Live Firebase render error:', e);
      HPL = FALLBACK;
      render(window.__page || 'home');
    }
  }, err => {
    console.error('Firebase live listener error:', err);
    // Keep the already loaded/fallback site usable if public Firestore access is unavailable.
    render(window.__page || 'home');
  });
}
