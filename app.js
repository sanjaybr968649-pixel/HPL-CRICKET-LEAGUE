...
{name:'Chasers',logo:'assets/chasers.png',captain:'Likith',ground:'Rajiv Gandhi International Cricket Stadium, Hyderabad'},
{name:'Stars',logo:'assets/stars.png',captain:'Karan',ground:'Sawai Mansingh International Stadium, Jaipur'}],
players:[
...
[6,'Stars','Chasers','170/5','238/5','Chasers won by 68 runs','Sawai Mansingh Stadium, Jaipur. Stadium, Jaipur'],
...
function logo(n){return (HPL.teams.find(t=>t.name===n)||{}).logo||({ 'Royal Kings':'assets/royal-kings.png','Titans':'assets/titans.png','Chasers':'assets/chasers.png','Stars':'assets/stars.png'}[n]||'assets/hpl.png')}
function header(p){return `<header class="header"><div class="head"><img class="hpl-logo" src="assets/hpl.png"><div><div class="title">HAND CRICKET PREMIER LEAGUE</div><div class="sub">Season 11 • Live Firebase Data</div></div></div></header><nav class="nav"><div class="navin">${[['home','Home'],['teams','Teams'],['players','Players'],['matches','Matches'],['points','Points Table'],['stats','Player Stats']].map(x=>`<button class="${p===x[0]?'active':''}" onclick="showPage('${x[0]}')">${x[1]}</button>`).join('')}</div></nav>`}
...
function st(t,sc,r){return `<div class="sc"><div class="sch">${esc(t)} — ${esc(sc)}</div>${(r||[]).map(x=>`<div class="sr"><span>${esc(x[0])}</span><span>${x[1]===null?'-':x[1]}</span><span class="wk">${x[2]?'│'.repeat(Number(x[2])):''}</span></div>`).join('')}</div>`}
